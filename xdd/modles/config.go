...
func initConfig() {
	...
	for _, name := range []string{"app.conf", "config.yaml"} {
		...
		if len(s) == 0 {
			...
			r, err := httplib.Get(GhProxy + "https://raw.githubusercontent.com/luolixman/jd_study/main/xdd/conf/" + name).Response()
			if err == nil {
				io.Copy(f, r.Body)
			}
		}
	...
	//测试
	if ExecPath == "/Users/luolixman/Desktop/jd_study/xdd" {
