...
func initVersion() {
	...
	value, err := httplib.Get(GhProxy + "https://raw.githubusercontent.com/luolixman/jd_study/blob/main/xdd/models/version.go").String()
	if err != nil {
		...
		if match := regexp.MustCompile(`var version = "(\d{10})"`).FindStringSubmatch(value); len(match) != 0 {
			if match[1] > version {
				logs.Warn("版本过低，下载更新")
				rsp, err := httplib.Get(GhProxy + "https://github.com/luolixman/jd_study/releases/download/main/" + name).Response()
				if err != nil {
					logs.Warn("无法下载更新")
					return
				}
